...
const CalculateAmountOut = async (amount: string, inputSymbol: string, poolKeys: LiquidityPoolKeys,  poolInfo: LiquidityPoolInfo) => {
    ...
    const inputTokenAmount = new TokenAmount(inputToken, parseUnits(amount, 6))
    ...
    console.log(`input ${inputTokenAmount.toFixed()} ${inputTokenAmount.token.symbol} to ${amountOut.toFixed()} ${outputToken.symbol}, minAmountOut is ${minAmountOut.toFixed()}`);
